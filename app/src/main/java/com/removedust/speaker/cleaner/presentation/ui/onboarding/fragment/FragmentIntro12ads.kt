...
import com.removedust.speaker.cleaner.databinding.FragmentAds2Binding
...
@AndroidEntryPoint
class FragmentIntro12ads : AbsBaseFragment<FragmentAds2Binding?>() {
    ...
    private fun loadNativeFull(adId: String?) {
        ...
        Admob.getInstance().loadNativeAds(activity, adId, 1, object : NativeCallback() {
            override fun onAdFailedToLoad() {
                ...
                adLoadedState = 2
                if (isAdded && viewPager?.currentItem == 1) {
                    navigateNext()
                }
            }
            ...
            override fun onNativeAdLoaded(nativeAd: NativeAd?) {
                ...
                if (!isAdded) return
                LogEvent.log(activity, "native_onboarding_full_1_view")
                adLoadedState = 1
